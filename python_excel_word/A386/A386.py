...
#根据指定参数(3.5)调整图像(myImage2)的色度，并将结果保存在temp目录中
ImageEnhance.Color(myImage2).enhance(3.5).save('temp/myImage2.jpg')
#根据temp/myImage2.jpg这个色度改变之后的图像(文件)创建图像(myNewImage2)
myNewImage2=openpyxl.drawing.image.Image('temp/myImage2.jpg')
#在生鲜订购表(mySheet)的B5单元格中添加色度改变之后的图像(myNewImage2)
