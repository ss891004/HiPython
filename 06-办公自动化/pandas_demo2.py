...
def fun2():
    ...
    df.年龄 # 使用点号选择列（仅适用于列名是有效Python标识符的情况）
    df.iloc[0]  # 选择第一行
    df.iloc[1:4]  # 选择第2-4行
    ...
    # 条件过滤
    df[df['年龄'] > 30] # 单条件过滤
    df[(df['年龄'] > 30) & (df['工资'] > 15000)]  # 多条件过滤（与）
    df[(df['城市'] == '北京') | (df['城市'] == '上海')]   # 多条件过滤（或）
    df[df['城市'].isin(['北京', '上海', '广州'])]  # 使用isin()过滤

    df.query('年龄 > 30 and 工资 > 15000')  # 使用query()方法（更直观的语法）
...
def fun3():
    ...
    df_missing.isnull().sum() # 检查缺失值
    df_clean1 = df_missing.dropna() # 删除包含任何缺失值的行 
    df_clean2 = df_missing.dropna(subset=['姓名', '年龄'])    # 删除特定列中有缺失值的行
    df_filled1 = df_missing.fillna({'姓名': '未知', '年龄': 0, '城市': '未知', '工资': 0})  # 填充缺失值 - 固定值
    df_filled2 = df_missing.fillna(method='ffill')  # 填充缺失值 - 前向填充
    df_filled3 = df_missing.fillna(method='bfill')    # 填充缺失值 - 后向填充

    df_filled4 = df_missing.copy()    # 填充缺失值 - 使用统计值
